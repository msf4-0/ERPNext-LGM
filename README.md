...
13. To access the custom LGM doctypes, make sure your browser is first open at the `/desk` page. For example, if the URL is `http://localhost:8000`, go to `http://localhost:8000/desk`. Then, select the "Projects" section, and open the Doctypes under "Lembaga Getah Malaysia". These menu options can be seen in the images at the top of this repo. Do note that you might need to manually go back to the desk page when opening the page sometimes, so please remember this step for future reference.
